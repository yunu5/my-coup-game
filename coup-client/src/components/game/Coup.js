    ...
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', flexWrap: 'wrap' }}>
        {this.state.players[this.state.playerIndex].influences.map((influence, index) => {
          return (
            <div key={index} className="InfluenceUnitContainer" style={{ textAlign: 'center' }}>
              <img 
                src={characterImages[influence.toLowerCase()]} 
                alt={influence} 
                style={{
                  width: '120px', 
                  height: '160px', 
                  objectFit: 'cover', 
                  borderRadius: '8px',
                  border: `3px solid ${this.influenceColorMap[influence.toLowerCase()]}` 
                }} 
              />
              <h3 style={{ textTransform: 'capitalize', marginTop: '5px' }}>{influence}</h3>
            </div>
          )
        })}
      </div>
